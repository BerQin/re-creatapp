...
function addSite(config){
  ...
  // 创建HTML入口文件
  const fpath = _path.join(publicPath,name+'.html');
  console.log(config, _path.join(TemplateModlePath,'HtmlModules.html'));
  _fs.readFile(_path.join(TemplateModlePath,'HtmlModules.html'),{
    encoding:'utf8'
  },function(err,data){
    if (err) throw err;
    console.log(data.replace(/\{\$name\}/g,name));
  });
  // creatFile(fpath,'');
}
